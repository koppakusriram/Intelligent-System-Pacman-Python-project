...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in allpositions:
            ...
            distribution[position] = self.beliefs[position] * observationprob;
    ...
    def elapseTime(self, gameState):
        ...
        allpositions = self.allPositions
        for position in allpositions:
            positiondistribution = self.getPositionDistribution(gameState, position)
            for newposition in positiondistribution.keys():
                distribution[newposition] += self.beliefs[position] * positiondistribution[newposition]
        self.beliefs = distribution
        #raiseNotDefined()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** CS5368 YOUR CODE HERE ***"
        for particles in range(self.numParticles):
            list = particles % len(self.legalPositions)
            self.particles.append(self.legalPositions[list])
        #raiseNotDefined()
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in self.particles:
            distribution[position] += self.getObservationProb(observation, pacmanposition, position, jailposition)
        if distribution.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.beliefs = distribution.normalize()
            for Particles in range(self.numParticles):
                self.particles[Particles] = distribution.sample()
        #raiseNotDefined()
    ...
    def elapseTime(self, gameState):
        ...
        for num in range(self.numParticles):
            a = self.particles[num]
            if a not in distribution.keys():
                position = self.getPositionDistribution(gameState, a)
                distribution[self.particles[num]] = position
                a = position.sample()
            else:
                a = self.particles[num]
                self.particles[num] = distribution[a].sample()
        #raiseNotDefined()
